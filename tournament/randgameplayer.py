...
class RandGamePlayer:
    ...
    def continueGame(self):
        ...
        while not resigned:
            ...
            self._game.addMove(whostr + " " + move)
    ...
    def save(self, fileName, name, resultBlack, resultWhite):
        ...
        for move in self._game.moveList():
            ...
            if move == "swap-pieces":
                ...
                f.write(move[0:1].upper()+ "[")
                f.write("resign]\n")
                break;
            else:
                f.write(move[0:1].upper() + "[" + move[2:] + "]")
            f.write("\n")
            
        f.write(")\n");
        f.close()

    def saveProject(self, fileName, name, resultBlack, resultWhite):
        t = time.localtime()
        sgfDate = time.strftime("%Y-%m-%d", t)
        longDate = time.strftime("%Y-%m-%d %X %Z", t)
        hostname = socket.gethostbyaddr(socket.gethostname())[0]
